...
test("full init wizard creates config, schema state, and manifest", async () => {
  await withTempDir(async (cwd) => {
    ...
    const prompter = createMockPrompter({
      text: ["http://localhost:4000", "my-project"],
      select: ["staging"],
...
test("init fails when server unreachable", async () => {
  await withTempDir(async (cwd) => {
    ...
    assert.ok(
      ...
      "stderr should mention server error status",
    );
  });
});
...
test("init with no content files still generates config", async () => {
  await withTempDir(async (cwd) => {
    ...
    const prompter = createMockPrompter({
      text: ["http://localhost:4000", "empty-project"],
      select: ["production"],
      multiSelect: [],
      confirm: [true],
    });
    ...
    const configContent = await readFile(configPath, "utf-8");
    assert.ok(
      configContent.includes("empty-project"),
      "config should contain project",
    );
  });
});
