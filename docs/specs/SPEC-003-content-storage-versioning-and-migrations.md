...
`GET /api/v1/content/:documentId/variants` returns all non-deleted locale variants that share the same `translation_group_id` as the given document. The response includes the queried document itself. For non-localized types the response contains a single entry.
