...
export type CliActionCatalogAdapterOptions = {
  headers?: ActionCatalogHeaders;
  fetcher?: (
...
export type CliActionCatalogAdapter = {
...
function toAdapterError(
  ...
  return new RuntimeError({
    ...
    message: `CLI action catalog ${action} request failed.`,
    statusCode: status,
    details: {
      status,
      error,
...
/**
 * createCliActionCatalogAdapter resolves `/api/v1/actions` contract data
 * through Eden/Treaty with shared runtime schema validation.
 */
export function createCliActionCatalogAdapter(
  baseUrl: string,
  options: CliActionCatalogAdapterOptions = {},
): CliActionCatalogAdapter {
